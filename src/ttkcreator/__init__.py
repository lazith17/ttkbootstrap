...
from ttkbootstrap.core.themes import ThemeColors, ThemeDefinition
...
class CreatorDesignWindow(tk.Toplevel):
    ...
    def get_selectors(self):
        ...
        """
        return self.children["window"].children["color_chooser"].children["selectors"].children
    ...
    def color_selector(self, master, color_label):
        """A container widget that represents a color selector. This includes a label, a color patch,
        an entry field, and a button for invoking the built-in color chooser.

        Args:
            master (Widget): the parent widget
            color_label (str): the name of the color represented by the selector (eg. primary, secondary,
                info, etc...)
    ...
    def select_color(self, selector, event=None):
        ...
        Args:
            selector (Widget): The widget container for a color selector
            event (Event): The event triggering the select color function
        """
    ...
    def create_variables(self):
        ...
        self.vars["font"] = tk.StringVar(name="font", value=self.style.theme.font)
        self.vars["type"] = tk.StringVar(name="type", value=self.style.theme.type)
    ...
    def update_theme(self, var, index, mode):
        """A callback function on the variable observer. Update existing theme if existing, otherwise, create
        a new theme and apply to app

        Args:
            var (str): The name of the tkinter variable observed
            index (int): The index of the item (if an array)
            mode (str): The mode of the trace observer
        """
        ...
        try:
            self.style.theme_definitions.update(
                {
                    theme_id: ThemeDefinition(
                        name=theme_id, themetype=self.getvar("type"), font=self.getvar("font"), colors=colors
                    )
                }
            )
            self.style.themes.add(theme_id)
...
class CreatorBaseChooser(tk.Tk):
    ...
    def setup(self):
        ...
        lbl = ttk.Label(
            self.frame, text="What kind of theme do you want to create?", font="-size 16 -slant italic"
        )
        ...
        light = ttk.Button(
            self.frame, text="Light", style="light.Outline.TButton", command=self.create_light_theme
        )
    ...
    def create_dark_theme(self):
        ...
        CreatorDesignWindow(self, "darkly")
        self.withdraw()
    ...
    def create_light_theme(self):
        ...
        CreatorDesignWindow(self, "flatly")
        self.withdraw()
    ...
    def check_user_themes_path(self):
        ...
        if not userpath:
            showwarning(
                title="User Defined Themes", message="Cannot save user-defined themes without a valid path"
            )
            return False
        else:
